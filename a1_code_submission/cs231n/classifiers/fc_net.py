...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        out1a, cache1a = affine_forward(X, self.params['W1'], self.params['b1'])
        out1b, cache1b = relu_forward(out1a)
        out2a, cache2a = affine_forward(out1b, self.params['W2'], self.params['b2'])
        scores = out2a

        # exps = np.exp(out2a - np.max(out2a, axis=1, keepdims=True))
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, dout2b = softmax_loss(scores, y)
        if self.reg != 0:
          loss += 0.5 * self.reg * ((self.params['W2'] ** 2).sum() + (self.params['W1'] ** 2).sum())
        dout2a, grads['W2'], grads['b2'] = affine_backward(dout2b, cache2a)
        dout1b = relu_backward(dout2a, cache1b)
        dout1a, grads['W1'], grads['b1'] = affine_backward(dout1b, cache1a)
